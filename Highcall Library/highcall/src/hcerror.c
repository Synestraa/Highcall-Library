...
HcErrorStatusTranslate(IN NTSTATUS Status)
{
	...
	while (table->start)
	{
		...
		{
			ErrorCode = table->table[Status - table->start];
			if (!ErrorCode)
			{
...
HcErrorSetNtStatus(IN NTSTATUS Status)
{
	...
	dwErrCode = HcErrorStatusTranslate(Status);
	HcErrorSetDosError(dwErrCode);
...
static VOID SetNoteA(IN LPCSTR lpNote)
{
	...
	if (!last_note_A.note)
	{
		...
		last_note_A.note = (LPSTR)HcAlloc(1024 * sizeof(CHAR) + sizeof(CHAR));
		last_note_A.size = 1024 * sizeof(CHAR) + sizeof(CHAR);
	}

	/* Get the size we need to copy. */
	sizeOfNewNote = HcStringSecureLengthA(lpNote);
...
static VOID SetNoteW(IN LPCWSTR lpNote)
{
	...
	{
		/* Max size of a note is 1kb. */
		last_note_W.note = (LPWSTR)HcAlloc(1024 * sizeof(WCHAR) + sizeof(WCHAR));
		last_note_W.size = 1024 * sizeof(WCHAR) + sizeof(WCHAR);
	}

	/* Get the size we need to copy. */
	sizeOfNewNote = HcStringSecureLengthW(lpNote);
